...
mod tests {
    ...
    async fn it_works() {
        ...
        let title = match item {
            crate::multi::MultiSearchResult::Movie(movie) => &movie.inner.title,
            crate::multi::MultiSearchResult::TVShow(tvshow) => &tvshow.inner.name,
            crate::multi::MultiSearchResult::Person(person) => &person.name,
        };
...
mod integration_tests {
    ...
    async fn search_rrrrrrr() {
        ...
        let result = client
            .search_multi("Rrrrrrr", &Default::default())
            .await
            .unwrap();
        assert_eq!(result.page, 1);
    ...
    async fn search_simpsons() {
        ...
        let client = Client::<ReqwestClient>::new(secret);
        let _result = client
            .search_multi("simpsons", &Default::default())
            .await
            .unwrap();
    }
}
