...
pub struct ListMoviesUpcomingParams<'a> {
...
impl<'a> ListMoviesUpcomingParams<'a> {
...
impl<E: crate::client::Executor> crate::Client<E> {
    ...
    /// }
    /// ```
    pub async fn list_movies_upcoming(
        &self,
        params: &ListMoviesUpcomingParams<'_>,
    ) -> crate::Result<PaginatedResult<super::MovieShort>> {
        self.execute("/movie/upcoming", params).await
    }
}
...
mod tests {
    ...
    async fn it_works() {
        ...
        let result = client
            .list_movies_upcoming(&Default::default())
            .await
            .unwrap();
